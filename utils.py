...
def detect_elan_files(folder_with_elan_files):
...
def parse_elan_file(elan_file, working_directory):
    ...
    for elan_filename, _ in zip(elan_file_list, progress_bar):
        ...
        try:
            ...
            tier_container, time_slots, media_data, _ = parse_elan_file(elan_file_path,
                                                           output_filename)    
            detected_tier_id += [eval(k)['TIER_ID'] for k in tier_container.keys()]
...
def align_annotations(working_dir, leading_modality):
...
def merge_modalities(working_dir, required_modalities, elan_file_list):
...
###############################################################################
def extracting_visual_features_v1(top_video_path, working_dir):
...
###############################################################################
def extracting_visual_features_v2(top_video_path, working_dir):
    video_extensions = ['mov', 'flv', 'mpg', 'mp4', 'avi', 'mpeg']
    video_paths = glob.glob(top_video_path+'/**/*', recursive = True)+glob.glob(top_video_path+'/**', recursive = True)
    video_paths = set([e for e in video_paths 
                    if  any([True if e.split('.')[-1].lower() == v_ext else False
...
###############################################################################
def extracting_visual_features_v3(top_video_path, working_dir):
    video_extensions = ['mov', 'flv', 'mpg', 'mp4', 'avi', 'mpeg']
    video_paths = glob.glob(top_video_path+'/**/*', recursive = True)+glob.glob(top_video_path+'/**', recursive = True)
    video_paths = set([e for e in video_paths 
                    if  any([True if e.split('.')[-1].lower() == v_ext else False
